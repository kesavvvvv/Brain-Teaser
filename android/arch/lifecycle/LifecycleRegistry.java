...
public class LifecycleRegistry extends Lifecycle {
  ...
  private Lifecycle.State calculateTargetState(LifecycleObserver paramLifecycleObserver) {
    ...
    if (!this.mParentStates.isEmpty()) {
      ...
      state = (Lifecycle.State)state.get(state.size() - 1);
    } 
    return min(min(this.mState, entry), state);
  }
  ...
  static Lifecycle.State min(@NonNull Lifecycle.State paramState1, @Nullable Lifecycle.State paramState2) { return (paramState2 != null && paramState2.compareTo(paramState1) < 0) ? paramState2 : paramState1; }
  ...
  private void pushParentState(Lifecycle.State paramState) { this.mParentStates.add(paramState); }
  ...
  @NonNull
  public Lifecycle.State getCurrentState() { return this.mState; }
  
  public int getObserverCount() { return this.mObserverMap.size(); }
  
  public void handleLifecycleEvent(@NonNull Lifecycle.Event paramEvent) { moveToState(getStateAfter(paramEvent)); }
  
  @MainThread
  public void markState(@NonNull Lifecycle.State paramState) { moveToState(paramState); }
  
  public void removeObserver(@NonNull LifecycleObserver paramLifecycleObserver) { this.mObserverMap.remove(paramLifecycleObserver); }
...
/* Location:              D:\Apks\New folder\classes-dex2jar.jar!\android\arch\lifecycle\LifecycleRegistry.class
