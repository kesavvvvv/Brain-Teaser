      ...
      if (b2) {
        ...
        if (b2 != b1) {
          ...
          while (true) {
            b3 = b1;
            if (j < paramInt2) {
              BackStackRecord backStackRecord = (BackStackRecord)paramArrayList.get(j);
              int k = backStackRecord.mOps.size();
              for (b3 = 0; b3 < k; b3++) {
                boolean bool;
                Op op1 = (Op)backStackRecord.mOps.get(b3);
...
/* Location:              D:\Apks\New folder\classes-dex2jar.jar!\android\support\v4\app\BackStackRecord.class
